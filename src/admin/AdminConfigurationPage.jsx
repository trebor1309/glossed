...
export default function AdminConfigurationPage() {
  ...
  const [payload, setPayload] = useState(JSON.stringify(templates.liquidity_limit, null, 2)); const [mfaCode, setMfaCode] = useState(""); const [needsMfa, setNeedsMfa] = useState(false);
  ...
  const submit = async (event) => { event.preventDefault(); setBusy(true); setError(null); try { await createAdminConfigurationVersion(type, version.trim(), JSON.parse(payload), reason.trim()); setVersion(""); setReason(""); await load(); } catch (e) { setError(e.message); setNeedsMfa(/MFA|permission/i.test(e.message)); } finally { setBusy(false); } };
  const reauthenticate = async () => { setBusy(true); setError(null); try { await verifyMfa(mfaCode, factors[0]?.id); setMfaCode(""); setNeedsMfa(false); } catch (e) { setError(e.message); } finally { setBusy(false); } };

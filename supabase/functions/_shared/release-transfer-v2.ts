...
async function contextForPayment(paymentId: string) {
  const { data, error } = await admin.rpc(
    "get_checkout_v2_release_connect_context",
    {
  ...
  if (!context?.stripe_account_id) {
    throw new Error("Connect account is unavailable");
  }
  return context;
}
...
export async function dispatchProviderTransferV2(
  paymentId: string,
  transferId: string,
) {
  const context = await refreshConnectForRelease(paymentId);
  const { data, error } = await admin.rpc(
    "reserve_provider_transfer_dispatch_v2",
    {
  ...
  if (!reservation) {
    throw new Error("Provider transfer reservation is unavailable");
  }
  ...
  const { error: submitError } = await admin.rpc(
    "mark_provider_transfer_submitted_v2",
    {
      p_transfer_id: transferId,
    },
  );

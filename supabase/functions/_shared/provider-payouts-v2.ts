...
function amountForCurrency(
  rows: Array<{ amount?: number; currency?: string; source_types?: unknown }> | undefined,
  currency: string
) {
...
function instantForCurrency(balance: Stripe.Balance, currency: string) {
  ...
  if (net > gross) throw new Error("Stripe returned an invalid Instant Payout balance");
  return {
    gross,
    net,
    destination: candidate?.destination ?? null,
    destinationType: candidate?.destination?.startsWith("card_") ? "card" : "bank_account",
    sourceTypes: row?.source_types ?? {},
  };
}
...
async function payoutContext(providerId: string) {
  ...
  if (!context?.stripe_account_id) throw new Error("Provider Connect account is unavailable");
  return context;
}

async function refreshConnectAccount(context: PayoutContext) {
  for (let pass = 0; pass < 2; pass += 1) {
    const current = pass === 0 ? context : await payoutContext(context.provider_id);
...
async function ensureManualSchedule(context: PayoutContext) {
  ...
  if (interval !== "manual") throw new Error("Stripe manual payout schedule was not applied");
  const { error } = await admin.rpc("record_provider_payout_schedule_control_v2", {
...
export async function refreshProviderBalanceV2(providerId: string) {
  ...
  const { data, error } = await admin.rpc("record_provider_balance_snapshot_v2", {
    p_stripe_account_id: context.stripe_account_id,
    p_currency: currency,
    ...
    p_instant_destination_type: instant.destination ? instant.destinationType : null,
    p_source_types: {
    ...
    p_retrieval_key: `provider-balance-v2:${context.stripe_account_id}:${crypto.randomUUID()}`,
  });
  if (error) throw error;
...
function errorDetails(error: unknown) {
  ...
  return {
    ...
    message: (error instanceof Error ? error.message : "Unknown payout failure").slice(0, 4000),
    retryable: true,
  };
}

export async function dispatchProviderPayoutV2(payoutId: string, providerId: string) {
  const snapshot = await refreshProviderBalanceV2(providerId);
  const { data, error } = await admin.rpc("reserve_provider_payout_dispatch_v2", {
    p_payout_id: payoutId,
    p_balance_snapshot_id: snapshot.id,
  });
  if (error) throw error;
  const reservation = data?.[0] as PayoutReservation | undefined;
  if (!reservation) throw new Error("Provider payout reservation is unavailable");
  ...
  try {
    const payout = await stripe.payouts.create(
      {
        ...
        ...(reservation.destination_id ? { destination: reservation.destination_id } : {}),
        metadata: {
    ...
    const applicationFee =
      typeof payout.application_fee === "string" ? payout.application_fee : payout.application_fee?.id;
    const balanceTransaction =
      typeof payout.balance_transaction === "string"
        ? payout.balance_transaction
        : payout.balance_transaction?.id;
    const { error: attachError } = await admin.rpc("attach_provider_payout_stripe_object_v2", {
    ...
    return { payout_id: reservation.payout_id, stripe_payout_id: payout.id, status: payout.status };
  } catch (error) {
    const details = errorDetails(error);
    const { error: failureError } = await admin.rpc("fail_provider_payout_dispatch_v2", {
      p_payout_id: reservation.payout_id,
...
export async function reserveAndDispatchStandardPayoutV2(providerId: string) {
  const snapshot = await refreshProviderBalanceV2(providerId);
  const { data, error } = await admin.rpc("reserve_standard_provider_payout_v2", {
    p_provider_id: providerId,
    p_balance_snapshot_id: snapshot.id,
  });
  if (error) throw error;
...
export async function confirmAndDispatchInstantPayoutV2(providerId: string, payoutId: string) {
  const { data, error } = await admin.rpc("confirm_provider_instant_payout_v2", {
    p_provider_id: providerId,
    p_payout_id: payoutId,
  });
  if (error) throw error;

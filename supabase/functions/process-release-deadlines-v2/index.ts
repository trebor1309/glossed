...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const requestedLimit = body && typeof body === "object" && "limit" in body
      ? Number(body.limit)
      : Number.NaN;
    ...
    for (const candidate of due ?? []) {
      try {
        ...
        const rpcName = candidate.release_trigger === "client_confirmation"
        ...
        if (reservation?.transfer_id) {
          ...
          results.push(
            await dispatchProviderTransferV2(
